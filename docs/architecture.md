...
| `mdux.medui.viewport` | `include/mdux/medui/Viewport.cppm` | `src/medui/Viewport.cpp` |
...
| `MduXMeduiLib` | `tools/medui/` | the `.medui` compiler (#15); the shared `MEDUI-E` diagnostic registry (#191), parser (#192), component/theme/locale semantic analyzer (#193), integer-only bounded layout solver (#194), the text-budget check that measures resolved boxes against the widest approved translation (#195), and the golden references that say where safety-critical content must appear (#196), the canonical package with its two C++ emitters (#197), the compiler driver behind `mdux-meduic` (#198), the machine-readable contract `--grammar` and `--explain` publish (#263), and the canonical `.medui` source serializer `mdux.tools.medui.serialize` (#325, [ADR-023](adr/ADR-023-medui-host-editing-api-and-round-trip-source-contract.md)) that completes the host editing API's round-trip direction, and the versioned AST JSON document `mdux.tools.medui.document` (#327, [ADR-025](adr/ADR-025-medui-studio-editing-and-reviewable-change-proposals.md)) with comment-loss and safety-metadata change detection |
| `MduXPreviewLib` | `tools/preview/` | Optional `mdux-preview` authenticated loopback service: request-owned source compilation, explicit fixture bindings and production offscreen PNG previews ([ADR-024](adr/ADR-024-local-native-preview-service.md)); the embedded MedUI Studio editor, document routes and opt-in change proposals built with git plumbing in a service-owned repository, never merged ([ADR-025](adr/ADR-025-medui-studio-editing-and-reviewable-change-proposals.md)) |
...
| `generated/scenario/endoscope-monitor-basics/` | `mdux-scenariobake`, then `mdux-verify-scenario-bake` | `scenario.json` + `scenario-verification.json` |
...
paints — pure functions `validate()` calls to prove a frame *could* be recorded before
`recordWaterfall()` turns that into one `addSolidRect()` per cell, all-or-nothing, exactly as
`recordTrace()` expands a proven `SampleRing`. `VulkanViewport` has never carried a governed colour
token — `fieldColorToken()` has no case for it, and the compiler's own golden-reference pass already
lists it beside `Image` and `Clock` as declaring none — so this module keeps that exclusion rather
than reversing it: the two-colour ramp `WaterfallStyle` carries is caller-supplied and unverified,
exactly as `TraceStyle`'s numeric range already is, and an *unbound* `VulkanViewport` therefore draws
nothing at all rather than an opaque field — there is no token to reserve one in. `ViewportBinding`
...
loaders — so the digest it records is of the bytes it actually replayed and rendered against. It
attests that run, not source-to-recipe identity: an internally inconsistent edit fails a binding
obligation here, while a same-id edit that stays consistent but never re-baked is a byte difference
in `evidence.scenario.<id>`. For each `capture` it renders the settled frame through `mdux.render.offscreen` and

...
| `MduXMeduiLib` | `tools/medui/` | the `.medui` compiler (#15); the shared `MEDUI-E` diagnostic registry (#191), parser (#192), component/theme/locale semantic analyzer (#193), integer-only bounded layout solver (#194), the text-budget check that measures resolved boxes against the widest approved translation (#195), and the golden references that say where safety-critical content must appear (#196), the canonical package with its two C++ emitters (#197), the compiler driver behind `mdux-meduic` (#198), the machine-readable contract `--grammar` and `--explain` publish (#263), and the canonical `.medui` source serializer `mdux.tools.medui.serialize` (#325, [ADR-023](adr/ADR-023-medui-host-editing-api-and-round-trip-source-contract.md)) that completes the host editing API's round-trip direction |
| `MduXPreviewLib` | `tools/preview/` | Optional `mdux-preview` authenticated loopback service: request-owned source compilation, explicit fixture bindings and production offscreen PNG previews; [ADR-024](adr/ADR-024-local-native-preview-service.md) |

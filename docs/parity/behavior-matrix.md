...
against `mdux.verify`'s own exported predicates (`rectContainedBy`, `inflate`, `couldBeBlend`,
`evidence::sha256`), plus the 10 `consumer-manifest` contract cases and committed negative
failure-mode fixtures, on all five CI legs. The `mdux.local/*` ids and the committed

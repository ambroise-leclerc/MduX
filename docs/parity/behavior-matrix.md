...
| VulkanViewport | stream source | MduX reserves geometry but still defers stream content; TrustSC paints a bounded waterfall. Row/bin order, numeric range/clamping and composition are now proposed by #322 (ADR-022, `mdux.medui.viewport`) — a caller-supplied, row-granularity grid and two-colour ramp, no schema extension. Binding it to a live screen and drawing it stays #323. |
...
| RegionPainted | Some pixel of a node's rectangle differs from the driver's resolved ground; blank fails, off-frame fails. The weakest honest claim for a scene-driven node (a live `NumericDisplay` value, a typed field) whose exact glyphs a host check cannot predict without on-device shaping (ADR-010). Named `mdux.local/region-painted` v1 since #321. | No sibling. | MduX-local; the pinned *value* is the scenario's binding obligation, not a rendered check (ADR-021 decision 4). |

...
layer and delivered the committed-artifact half of the ADR-017 §3 rendered-id migration: each mapped
rendered-check outcome in `verification.json` now records a candidate `candidateProfile`
(`MEDUI-PROFILE-RENDERED` `{profile, check}`) **beside** its retained `mdux.local/`
`observationProfile`, so the bundle runs both identities together. `mdux.local/ink-coverage`
(`LocalizedTextPresence`) maps to nothing and stays implementation-local. No check semantics, no
finding and no runtime behaviour change. This was done ahead of the ADR-016 gate (a final 0.3.0
...
| ADR-016 (local observation profiles + #313 candidate migration) | MduX maintainer; domain reviewer (safety-relevant verifier area) | **Accepted 2026-09-09 (#313)** for the engineering layer, by maintainer instruction; verifier-area domain review still open. The four required rendered checks (`Bounds`, `ColorHash`, `InkContainment`, `LocalizedTextPresence`) keep their exact semantics; each gains an implementation-local `ObservationProfile` id recorded per outcome, and — since the #313 amendment — a mapped check also records a candidate `candidateProfile` (`MEDUI-PROFILE-RENDERED` `{profile, check}`) beside it, derived through one pure `canonicalRenderedCheckFor()`. `mdux.verify::rawImageDigest()` is an exported, fixture-tested predicate with a defined `NoBaseline` outcome that never discharges an obligation — **not** a `CvCheck`, **no production caller**, **outside driver obligation enumeration**. Production-driver behaviour is unchanged; the committed `verification.json` differs only by the additive `observationProfile` / `candidateProfile` members, and no finding changes. Residual: final 0.3.0 pin, #335 joint sign-off, reviewed re-bake. |

...
| #316 queue/editing; #317 presentation adapter | Accepted #315 requirements (ADR-018); D5; PAR-REQ-004–007. **#316 delivered** `EventQueue` and `FieldEditor` in `mdux.medui.input` (`medui_spec` + `input_noheap_spec`). #317 supplies the platform adapter and the coordinate-scale source that fill the queue. |
...
`FieldEditor`; the platform adapter is #317, and the assembled update loop and its replay are
#318/#320. It assigns no clinical hazard, severity or risk control, and PAR-REQ-009/010 keep
...
| [ADR-018](../adr/ADR-018-bounded-input-and-update-order.md) (bounded input + update order + action policy) | MduX maintainer; domain reviewer (critical-action host policy, PAR-REQ-006) | **Accepted by Ambroise Leclerc, 2026-09-08** (#315). Defines `mdux.medui.input` — the closed event vocabulary, `maxInputEvents`, floor coordinate normalization, `PressLatch`, `EventQueue`, `FieldEditor`, the update order, and `ActionTrace`. Ratifies PAR-REQ-004 Accepted, PAR-REQ-005 Accepted with amendment, PAR-REQ-006 Accepted with a recorded limitation, PAR-REQ-007 Accepted, PAR-REQ-008 Accepted with amendment. #316 delivered `EventQueue` + `FieldEditor`. The critical-action host execution / audit policy stays for domain review; the assembled update loop is #318. |
| PAR-REQ-001–010 | MduX maintainer; domain reviewer for any device action/risk-control interpretation | PAR-REQ-001/002/003: engineering half gated in CI by #314; **dispositions ratified 2026-09-08** via ADR-017. #313 delivered the **local** half of PAR-REQ-002 and, on 2026-09-09, the committed `candidateProfile` migration beside it for `Bounds`/`ColorHash` (ADR-016 #313 amendment); #314 Stage B the **shared-corpus** half of PAR-REQ-002, Stage C the shared-corpus half of PAR-REQ-003, Stage C-emitter MduX's own derived envelope. **PAR-REQ-004–008: dispositions ratified 2026-09-08** via ADR-018, tied to `mdux.medui.input` + `InputContractTests`. #316 delivered `EventQueue` and `FieldEditor` (PAR-REQ-005/007). Still open: PAR-REQ-006's host policy (domain review), the assembled update loop (#318) and its replay (#320). PAR-REQ-009–010 remain Proposed / unreviewed. The cross-implementation half of the verifier requirements is [#335](https://github.com/ambroise-leclerc/MduX/issues/335). |

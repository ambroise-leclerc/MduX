...
| §8.2 | [Monitoring and measurement](06-measurement-analysis-improvement.md#82-monitoring-and-measurement) | The evidence-kernel and host-tools test suites (91 cases as of the epic that introduced them) run on every push to main or develop and on every pull request whose base branch is main, develop or feat/**, across the two toolchains automatic CI exercises - MSVC and GCC 16; a Clang workflow exists but is manual-dispatch only, so it contributes no automatic coverage - giving continuous, reproducible monitoring of whether the codebase still meets its own specified behaviour, not a periodic audit sample. | JUS-010 |

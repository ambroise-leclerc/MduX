...
private:
    ...
    void createGraphicsPipeline() {
        ...
        auto createShaderModule = [this](span<const byte> spirv,
                                         string_view moduleId) -> VkShaderModule {
            // PackageView::moduleSpirv() answers an empty span for an id the package does not
            // contain, and for one whose recorded range does not fit the sidecar. Passing that
            // straight on builds a createInfo with codeSize 0, and the only thing the failure
            // then says is "failed to create shader module" - which sends the reader looking at
            // Vulkan when the fault is a mistyped id or a corrupted package. Checked here so the
            // message names the module and the reason.
            if (spirv.empty()) {
                throw runtime_error("shader package has no module '" + string{moduleId} +
                                    "', or its recorded range does not fit the package payload");
            }
            if (spirv.size() % 4 != 0) {
                throw runtime_error("module '" + string{moduleId} + "' is " +
                                    to_string(spirv.size()) +
        ...
        VkShaderModule vertShaderModule = createShaderModule(package.moduleSpirv("triangle.vert"), "triangle.vert");
        VkShaderModule fragShaderModule = createShaderModule(package.moduleSpirv("triangle.frag"), "triangle.frag");

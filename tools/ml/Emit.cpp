...
/// @brief Appends one stable host-tool diagnostic.
...
/// @brief Reads a file without text-mode translation.
...
/// @brief Escapes one string for a generated C++ string literal.
...
/// @brief Renders an enum through its canonical wire-value table, with a bounds check.
template <typename Enum, std::size_t Size>
[[nodiscard]] std::string renderEnumerator(std::string_view typeName, Enum value, const std::array<std::string_view, Size>& wireValues) {
    const std::size_t index = static_cast<std::size_t>(value);
    if (index >= wireValues.size()) {
        throw std::logic_error{std::format("{} value {} has no canonical wire spelling", typeName, index)};
    }

    std::string spelling{wireValues[index]};
    if (!spelling.empty() && spelling.front() >= 'a' && spelling.front() <= 'z') {
        spelling.front() = static_cast<char>(spelling.front() - 'a' + 'A');
    }
    return std::format("mdux::ml::{}::{}", typeName, spelling);
}

/// @brief Renders a layer-kind enumerator from the schema's load-bearing wire table.
[[nodiscard]] std::string renderLayerKind(schema::LayerKind kind) {
    return renderEnumerator("LayerKind", kind, schema::layerKindWireValues);
}

/// @brief Renders an activation enumerator from the schema's load-bearing wire table.
[[nodiscard]] std::string renderActivation(schema::Activation activation) {
    return renderEnumerator("Activation", activation, schema::activationWireValues);
}

/// @brief Renders one tensor reference as a designated initializer.
...
/// @brief Renders one golden-vector bit table.
...
/// @brief Renders the common namespace body shared by the module and header forms.
[[nodiscard]] std::string renderBody(const schema::ModelPackage& package, std::string_view identifier) {
    ...
    out += "/// @brief Returns the compile-time-validated model package.\n";
...
/// @brief Renders the provenance comment shared by both generated forms.
...
/// @brief Implements identifierForModel() from the emitter interface.
...
/// @brief Implements renderModel() from the emitter interface.
...
/// @brief Implements writeModel() from the emitter interface.

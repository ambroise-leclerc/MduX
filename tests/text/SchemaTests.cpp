...
const mdux::spec::Register referencePackageValidates{
    "The reference package validates", "evidence-unit", [] {
        return speclab::Test<ValidationErrorState>("text-reference-package-validates")
...
const mdux::spec::Register wrongKindRejected{
    "A package of the wrong kind is rejected", "evidence-unit", [] {
        return speclab::Test<ValidationErrorState>("text-wrong-kind-rejected")
            .Given("a package whose kind is not text", [](ValidationErrorState& state) {
...
const mdux::spec::Register unsupportedSchemaVersionRejected{
    "A package with the wrong schemaVersion is rejected", "evidence-unit", [] {
        ...
        return speclab::Test<ValidationErrorState>("text-unsupported-schema-version-rejected")
            .Given("a package whose schemaVersion is not the current one", [](ValidationErrorState& state) {
                TextPackage package = validPackage();
...
const mdux::spec::Register atlasIdRequired{
    "An empty atlas id is rejected", "evidence-unit", [] {
        return speclab::Test<ValidationErrorState>("text-empty-atlas-rejected")
            .Given("a package with an empty atlas", [](ValidationErrorState& state) {
                TextPackage package = validPackage();
...
const mdux::spec::Register localeRequired{
    "An empty locale is rejected", "evidence-unit", [] {
        return speclab::Test<ValidationErrorState>("text-empty-locale-rejected")
            .Given("a package with an empty locale", [](ValidationErrorState& state) {
                TextPackage package = validPackage();
...
const mdux::spec::Register sidecarPathBareFilename{
    "A sidecar path must be a bare filename", "evidence-unit", [] {
        return speclab::Test<SidecarPathBareFilenameState>("text-sidecar-path-bare-filename")
...
const mdux::spec::Register runsIdentityRequired{
    "A run with an empty id is rejected", "evidence-unit", [] {
        return speclab::Test<ValidationErrorState>("text-run-id-required")
            .Given("a package with an empty run id", [](ValidationErrorState& state) {
                TextPackage package = validPackage();
...
const mdux::spec::Register duplicateRunIdRejected{
    "Two runs sharing an id are rejected", "evidence-unit", [] {
        return speclab::Test<ValidationErrorState>("text-duplicate-run-id-rejected")
            .Given("a package where two runs share an id", [](ValidationErrorState& state) {
                TextPackage package = validPackage();
...
const mdux::spec::Register runAlignmentRequired{
    "A run whose byteLength is not a multiple of recordSize is rejected", "evidence-unit", [] {
        return speclab::Test<ValidationErrorState>("text-run-alignment-required")
            .Given("a package with a misaligned run length", [](ValidationErrorState& state) {
                TextPackage package = validPackage();
...
const mdux::spec::Register runOutOfBoundsRejected{
    "A run whose range extends past the sidecar is rejected", "evidence-unit", [] {
        return speclab::Test<ValidationErrorState>("text-run-out-of-bounds-rejected")
            .Given("a package with a run past the end of its sidecar", [](ValidationErrorState& state) {
                TextPackage package = validPackage();
...
const mdux::spec::Register overlappingRunsRejected{
    "Two overlapping run ranges are rejected", "evidence-unit", [] {
        return speclab::Test<ValidationErrorState>("text-overlapping-runs-rejected")
            .Given("a package where two runs share bytes", [](ValidationErrorState& state) {
                TextPackage package = validPackage();
...
const mdux::spec::Register emptyRunListAllowed{
    "An empty run list is valid", "evidence-unit", [] {
        ...
        return speclab::Test<ValidationErrorState>("text-empty-run-list-allowed")
            .Given("a package with no runs and a zero-byte sidecar", [](ValidationErrorState& state) {
                TextPackage package = validPackage();
                package.runs.clear();
...
const mdux::spec::Register packageRoundTrip{
    "A text package survives write() and parse() unchanged", "evidence-unit", [] {
        return speclab::Test<PackageRoundTripState>("text-package-round-trip")
            .Given("the reference package", [](PackageRoundTripState& state) {
                TextPackage package = validPackage();
            ...
            .Then("every field survives and the bytes are reproduced exactly",
                   [](PackageRoundTripState& state) {
                        ...
                        checks.expect(package.sidecarSha256 == digestOf("sidecar"),
                                       "sidecar digest");
                        Assertions::require(package.runs.size() == 2, "expected 2 runs, got {}", package.runs.size());
...
struct MalformedPackageRejectedState {
...
const mdux::spec::Register malformedPackageRejected{
    "Parsing rejects malformed text", "evidence-unit", [] {
        return speclab::Test<MalformedPackageRejectedState>("text-malformed-package-rejected")
            .Given("several malformed package texts", [](MalformedPackageRejectedState& state) {
                // Not JSON at all.
            ...
            .Then("each is rejected as MalformedPackage",
                   [](MalformedPackageRejectedState& state) {
                        mdux::spec::Checks checks;
...
const mdux::spec::Register findLocatesRun{
    "find() locates a run by id and returns null for absent ids", "evidence-unit", [] {
        ...
        return speclab::Test<FindLocatesRunState>("text-find-locates-run")
            .Given("the reference package", [](FindLocatesRunState& state) {
...
const mdux::spec::Register viewFindsRunById{
    "PackageView::find() locates a run by id and reports a miss", "evidence-unit", [] {
        return speclab::Test<ViewFindsRunByIdState>("text-view-find-locates-run")
            .Given("a view with two runs", [](ViewFindsRunByIdState& state) {
                state.runs = {
...
struct ViewRunBytesForPresentRunState {
...
const mdux::spec::Register viewRunBytesForPresentRun{
    "PackageView::runBytes() returns the slice for a present run", "evidence-unit", [] {
        return speclab::Test<ViewRunBytesForPresentRunState>("text-view-run-bytes-present")
            .Given("a view whose sidecar holds two runs", [](ViewRunBytesForPresentRunState& state) {
            ...
            .When("runBytes() is called for a present and an absent id", [](ViewRunBytesForPresentRunState& state) {
            ...
            .Then("the present slice matches the sidecar bytes and the absent slice is empty",
                   [](ViewRunBytesForPresentRunState& state) {
                        mdux::spec::Checks checks;
...
const mdux::spec::Register viewRunBytesOutOfBoundsReturnsEmpty{
    ...
    "evidence-unit", [] {
        ...
        return speclab::Test<RunViewState>("text-view-run-bytes-out-of-bounds")
            .Given("a view whose run points past the end of its sidecar", [](RunViewState& state) {
            ...
            .When("runBytes() is called", [](RunViewState& state) {
...
const mdux::spec::Register viewRunBytesOffsetPastEndReturnsEmpty{
    ...
    "evidence-unit", [] {
        ...
        return speclab::Test<RunViewState>("text-view-run-bytes-offset-past-end")
            .Given("a view whose run offset alone exceeds the sidecar length", [](RunViewState& state) {
                state.sidecar = {std::byte{1}, std::byte{2}, std::byte{3}, std::byte{4}};

...
[[nodiscard]] std::optional<std::vector<Chunk>> readPng(std::span<const std::byte> bytes) {
    ...
    while (offset + 12 <= bytes.size()) {
        ...
        chunks.push_back(Chunk{
            .type = std::string{    typeAndBody.begin(), typeAndBody.begin() + 4},
...
const mdux::spec::Register dimsAndOutlines{
    "A diff image dims the frame and outlines expected and found",
    "evidence-unit",
    [] {
        return speclab::Test("verify-ui-diff-outlines")
            ...
            .Then("the frame is dim, the expected box is magenta and the found box is cyan",
                  [] {
                      ...
                      const std::array        marks{
                          vu::DiffMark{.nodeId     = "readout",
                                       .check      = "Bounds",
                                                       ...
                                                       return speclab::Test("verify-ui-diff-extent")
                                                           ...
                                                           .Then("both return nothing rather than reading past the frame",
                                                                 [] {
                                                                     ...
                                                                     mdux::spec::Checks            assertions;
                                                                     assertions.expect(vu::composeDiff(tooFew, 8, 8, {}).empty(),
                                                                                       "compose refuses a short frame");
                                                                     assertions.expect(vu::encodePng(tooFew, 8, 8).empty(), "encode refuses a short frame");
                                                                     assertions.expect(vu::encodePng(tooFew, 0, 0).empty(),
                                                                                       "and refuses an empty extent, which no PNG can express");
                                                                     assertions.raise();
                                                                 })
                                                           .Execute();
                                                   }};

const mdux::spec::Register scopeNamesDoNotCollide{
    "Two scopes of one screen cannot claim the same diff image",
    "evidence-unit",
    [] {
        return speclab::Test("verify-ui-diff-name")
            ...
            .Then("they stay distinct, and an ordinary locale tag is unchanged",
                  [] {
                      ...
                      assertions.expect(vu::diffImageName("s", "en/US") != vu::diffImageName("s", "enUS"), "a separator is escaped rather than dropped");
...
const mdux::spec::Register frameAndDiffNamesDoNotCollide{
    "A scope's frame image and its diff image cannot claim the same name",
    "evidence-unit",
    [] {
        return speclab::Test("verify-ui-frame-name")
            .Given("one screen and one scope, named for both purposes", [] {})
            .When("the two filenames are formed", [] {})
            .Then("they differ, and the frame name encodes its scope the same injective way",
                  [] {
                      mdux::spec::Checks assertions;

                      // The arrangement a person actually types is one directory for both, and the
                      // failure that would produce if these collided is the worst kind available: a
                      // reader opening what they believe is the frame and seeing the dimmed, outlined
                      // one, with nothing in the filename to tell them which they have.
                      assertions.expect(vu::frameImageName("endoscope-monitor", "en-US") == "endoscope-monitor.en-US.frame.png",
                                        "an approved locale tag comes out as itself, under its own suffix");
                      assertions.expect(vu::frameImageName("s", "en-US") != vu::diffImageName("s", "en-US"), "the two images of one scope are two files");

                      // The encoding is shared rather than reimplemented, so this is the same property
                      // the diff name has, asked of the name that would otherwise be the one to drift.
                      assertions.expect(vu::frameImageName("s", "en/US") != vu::frameImageName("s", "enUS"), "a separator is escaped rather than dropped");
                      assertions.expect(vu::frameImageName("s", "(locale-free)") != vu::frameImageName("s", "locale-free"),
                                        "the locale-free scope cannot be confused with a locale of that name");

                      const std::string awkward = vu::frameImageName("s", "a/b\\c");
                      assertions.expect(awkward.find('/') == std::string::npos && awkward.find('\\') == std::string::npos,
                                        std::format("no path separator reaches the filename, got '{}'", awkward));
                      assertions.raise();
                  })
...
const mdux::spec::Register encodesRealPng{
    "The encoder writes a PNG a decoder reads back pixel for pixel",
    "evidence-unit",
    [] {
        return speclab::Test("verify-ui-diff-png")
            ...
            .Then("the raster that comes back is the one that went in",
                  [] {
                      ...
                      const std::uint32_t stated = (static_cast<std::uint32_t>(trailer[0]) << 24U) | (static_cast<std::uint32_t>(trailer[1]) << 16U)

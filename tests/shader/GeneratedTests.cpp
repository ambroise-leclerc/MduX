...
/// The same package as the module and the header expose it.
struct ModuleAndHeaderViewsState {
...
const mdux::spec::Register moduleHeaderByteIdentical{
    "The module and header forms expose byte-identical SPIR-V", "evidence-unit", [] {
        return speclab::Test<ModuleAndHeaderViewsState>("shader-gen-module-header-byte-identical")
            .Given("both generated forms of the UI package",
                   [](ModuleAndHeaderViewsState& state) {
                       state.fromM = fromModule();
                       state.fromH = fromHeader();
                   })
            .When("their payloads are compared", [](ModuleAndHeaderViewsState& state) {
                Assertions::require(state.fromM.spirv.size() == state.fromH.spirv.size(),
                                    "the module form has {} bytes, the header form {}",
                                    state.fromM.spirv.size(),
                                    state.fromH.spirv.size());
            })
...
const mdux::spec::Register moduleHeaderContractIdentical{
    "The module and header forms expose an identical contract", "evidence-unit", [] {
        return speclab::Test<ModuleAndHeaderViewsState>("shader-gen-module-header-contract-identical")
            .Given("both generated forms of the UI package",
                   [](ModuleAndHeaderViewsState& state) {
                       state.fromM = fromModule();
                       state.fromH = fromHeader();
                   })
            .When("their contracts are compared", [](ModuleAndHeaderViewsState& state) {
                if (state.fromM.modules.size() != state.fromH.modules.size() ||
                    ...
                    Assertions::fail("the two forms declare different contract sizes");
                }
            })
...
/// The committed package beside the view generated from it.
struct CommittedAndGeneratedState {
...
const mdux::spec::Register generatedMatchesCommitted{
    ...
    [] {
        return speclab::Test<CommittedAndGeneratedState>("shader-gen-generated-matches-committed")
            .Given("the committed UI package and the generated module form",
                   [](CommittedAndGeneratedState& state) {
                       state.committed = requireCommitted(committedPackage());
                       state.view = fromModule();
                   })
            .When("the generated view is compared against the committed package", [](CommittedAndGeneratedState& state) {
                if (state.view.modules.size() != state.committed.modules.size() ||
                    ...
                    Assertions::fail("the generated view and the committed package differ in size");
                }
            })
...
const mdux::spec::Register generatedRangesMatchDigests{
    "Each module's generated range matches the digest the package records", "evidence-unit", [] {
        return speclab::Test<CommittedAndGeneratedState>("shader-gen-generated-ranges-match-digests")
            .Given("the committed UI package and the generated module form",
                   [](CommittedAndGeneratedState& state) {
                       state.committed = requireCommitted(committedPackage());
                       state.view = fromModule();
                   })
            .When("each module's range is read from the view", [](CommittedAndGeneratedState& state) {
                for (const shader::ShaderModule& module : state.committed.modules) {
                    ...
                    Assertions::require(!bytes.empty(), "module '{}' produced an empty range", module.id);
                }
            })
...
const mdux::spec::Register identifierParity{
    "The generated identifier matches what the build predicts", "evidence-unit", [] {
        return speclab::Test<IdentifierParityState>("shader-gen-identifier-parity")
            ...
            .When("each CMake-computed identifier is compared against identifierFor()",
                  [](IdentifierParityState& state) {
                      ...
                      Assertions::require(parity.is_open(), "the parity file could not be opened");
                      ...
                      while (std::getline(parity, line)) {
                          ...
                          Assertions::require(tab != std::string::npos, "a parity line had no tab separator");

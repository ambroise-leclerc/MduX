...
const mdux::spec::Register wireRoundTrips{
    "Every wire spelling round-trips through its enum", "evidence-unit", [] {
        return speclab::Test<WireRoundTripsState>("shader-wire-round-trips")
            .Given("the wire value arrays in declaration order", [] {})
            .When("each wire value is converted back to its enum", [](WireRoundTripsState& state) {
                for (std::size_t i = 0; i < stageWireValues.size(); ++i) {
                    auto stage = stageFromWire(stageWireValues[i]);
                    Assertions::require(stage.has_value(), "stageFromWire('{}') rejected its own spelling", stageWireValues[i]);
                ...
                for (std::size_t i = 0; i < descriptorKindWireValues.size(); ++i) {
                    ...
                    Assertions::require(kind.has_value(), "descriptorKindFromWire('{}') rejected its own spelling", descriptorKindWireValues[i]);
...
const mdux::spec::Register unknownWireValueRejected{
    "An unknown wire value is rejected rather than defaulted", "evidence-unit", [] {
        return speclab::Test<UnknownWireValueRejectedState>("shader-unknown-wire-value-rejected")
            .Given("wire values outside the published spellings", [] {})
            .When("each is decoded", [](UnknownWireValueRejectedState& state) {
                auto stage = stageFromWire("compute");
                Assertions::require(!stage.has_value(), "stageFromWire('compute') succeeded");
                state.stageError = stage.error();

                auto kind = descriptorKindFromWire("inputAttachment");
                Assertions::require(!kind.has_value(), "descriptorKindFromWire('inputAttachment') succeeded");
                state.kindError = kind.error();
            })
            .Then("each is rejected with its own SchemaError",
                  [](UnknownWireValueRejectedState& state) {
                      mdux::spec::Checks checks;
...
struct StageMaskEnumeratorOrderState {
...
const mdux::spec::Register stageMaskEnumeratorOrder{
    "A stage mask is written in enumerator order, not assembly order", "evidence-unit", [] {
        return speclab::Test<StageMaskEnumeratorOrderState>("shader-stage-mask-enumerator-order")
            .Given("the same stage set built in two assembly orders", [] {})
            .When("both are serialized and one is decoded",
                  [](StageMaskEnumeratorOrderState& state) {
                      ...
                      Assertions::require(a.has_value(), "stagesToJson(vertex-first) did not serialize");
                      Assertions::require(b.has_value(), "stagesToJson(fragment-first) did not serialize");
                      state.sameBytes = (*a == *b);

                      auto mask = stagesFromJson(fromVertexFirst);
                      Assertions::require(mask.has_value(), "stagesFromJson rejected its own output");
                      state.decoded = *mask;
                  })
...
struct EmptyStageListDecodesState {
    StageMask mask{0xff};
};

const mdux::spec::Register emptyStageListDecodes{
    "An empty stage list decodes to an empty mask", "evidence-unit", [] {
        return speclab::Test<EmptyStageListDecodesState>("shader-empty-stage-list-decodes")
            .Given("an empty stage list", [] {})
            .When("it is serialized and decoded", [](EmptyStageListDecodesState& state) {
                auto mask = stagesFromJson(stagesToJson(0));
                Assertions::require(mask.has_value(), "stagesFromJson rejected an empty list");
                state.mask = *mask;
            })
            .Then("the mask is zero",
                  [](EmptyStageListDecodesState& state) {
                      mdux::spec::Checks checks;
...
/// What a rejection scenario keeps: the error `errorOf()` reported, or nothing.
...
const mdux::spec::Register referencePackageValidates{
    "The reference package validates", "evidence-unit", [] {
        return speclab::Test<ValidationErrorState>("shader-reference-package-validates")
            .Given("the reference package", [](ValidationErrorState& state) { state.error = errorOf(validPackage()); })
            .When("it is validated", [] {})
            .Then("no error is reported",
                  [](ValidationErrorState& state) {
...
const mdux::spec::Register wrongKindRejected{
    "A package of the wrong kind is rejected", "evidence-unit", [] {
        return speclab::Test<ValidationErrorState>("shader-wrong-kind-rejected")
            .Given("a package whose kind is not shader", [](ValidationErrorState& state) {
...
struct SidecarPathBareFilenameState {
...
const mdux::spec::Register sidecarPathBareFilename{
    "A sidecar path must be a bare filename", "evidence-unit", [] {
        return speclab::Test<SidecarPathBareFilenameState>("shader-sidecar-path-bare-filename")
            .Given("sidecar paths that are not bare filenames", [](SidecarPathBareFilenameState& state) {
                ShaderPackage package = validPackage();
            ...
            .Then("each is rejected with the right SchemaError",
                  [](SidecarPathBareFilenameState& state) {
                      mdux::spec::Checks checks;
...
const mdux::spec::Register noModulesRejected{
    "A package with no modules is rejected", "evidence-unit", [] {
        return speclab::Test<ValidationErrorState>("shader-no-modules-rejected")
            .Given("a package with no modules", [](ValidationErrorState& state) {
                ShaderPackage package = validPackage();
...
struct ModuleIdentityRequiredState {
    std::optional<SchemaError> empty;
...
const mdux::spec::Register moduleIdentityRequired{
    "Module identity is required and must be unique", "evidence-unit", [] {
        return speclab::Test<ModuleIdentityRequiredState>("shader-module-identity-required")
            .Given("modules with an empty id and a duplicated id", [](ModuleIdentityRequiredState& state) {
                ShaderPackage package = validPackage();
            ...
            .When("each is validated", [] {})
            .Then("each is rejected with the right SchemaError",
                  [](ModuleIdentityRequiredState& state) {
                      mdux::spec::Checks checks;
...
struct ModuleEntryPointAndLengthState {
...
const mdux::spec::Register moduleEntryPointAndLength{
    "A module needs an entry point and a non-zero length", "evidence-unit", [] {
        return speclab::Test<ModuleEntryPointAndLengthState>("shader-module-entry-point-and-length")
            .Given("modules with no entry point and with a zero length", [](ModuleEntryPointAndLengthState& state) {
                ShaderPackage package = validPackage();
            ...
            .When("each is validated", [] {})
            .Then("each is rejected with the right SchemaError",
                  [](ModuleEntryPointAndLengthState& state) {
                      mdux::spec::Checks checks;
...
struct ModuleRangesWordAlignedState {
...
const mdux::spec::Register moduleRangesWordAligned{
    "Module ranges must be word-aligned", "evidence-unit", [] {
        return speclab::Test<ModuleRangesWordAlignedState>("shader-module-ranges-word-aligned")
            .Given("modules with a misaligned length and offset", [](ModuleRangesWordAlignedState& state) {
            ...
            .Then("each is rejected as UnalignedModule",
                  [](ModuleRangesWordAlignedState& state) {
                      mdux::spec::Checks checks;
...
struct ModuleRangeWithinSidecarState {
...
const mdux::spec::Register moduleRangeWithinSidecar{
    "A module range may not extend past the sidecar", "evidence-unit", [] {
        return speclab::Test<ModuleRangeWithinSidecarState>("shader-module-range-within-sidecar")
            .Given("a range past the sidecar and one that would wrap", [](ModuleRangeWithinSidecarState& state) {
                ShaderPackage package = validPackage();
            ...
            .Then("each is rejected as ModuleOutOfBounds",
                  [](ModuleRangeWithinSidecarState& state) {
                      mdux::spec::Checks checks;
...
const mdux::spec::Register moduleRangesNoOverlap{
    "Two module ranges may not overlap", "evidence-unit", [] {
        return speclab::Test<ValidationErrorState>("shader-module-ranges-no-overlap")
            .Given("two modules sharing bytes", [](ValidationErrorState& state) {
...
const mdux::spec::Register adjacentRangesNotOverlap{
    "Adjacent module ranges are not an overlap", "evidence-unit", [] {
        return speclab::Test<ValidationErrorState>("shader-adjacent-ranges-not-overlap")
            .Given("two modules with exactly adjacent ranges", [](ValidationErrorState& state) {
...
const mdux::spec::Register descriptorNoStagesRejected{
    "A descriptor visible to no stage is rejected", "evidence-unit", [] {
        return speclab::Test<ValidationErrorState>("shader-descriptor-no-stages-rejected")
            .Given("a descriptor visible to no stage", [](ValidationErrorState& state) {
                ShaderPackage package = validPackage();
...
struct DescriptorCountAndUniquenessState {
...
const mdux::spec::Register descriptorCountAndUniqueness{
    "A descriptor needs a non-zero count and a unique binding", "evidence-unit", [] {
        return speclab::Test<DescriptorCountAndUniquenessState>("shader-descriptor-count-and-uniqueness")
            .Given("a descriptor with a zero count and two with the same binding", [](DescriptorCountAndUniquenessState& state) {
                ShaderPackage package = validPackage();
            ...
            .When("each is validated", [] {})
            .Then("each is rejected with the right SchemaError",
                  [](DescriptorCountAndUniquenessState& state) {
                      mdux::spec::Checks checks;
...
const mdux::spec::Register sameBindingDifferentSet{
    "The same binding number in a different set is not a duplicate", "evidence-unit", [] {
        return speclab::Test<ValidationErrorState>("shader-same-binding-different-set")
            .Given("the same binding number in two sets", [](ValidationErrorState& state) {
                ShaderPackage package = validPackage();
                package.descriptors.push_back(DescriptorBinding{.set = 0,
                                                                .binding = 1,
                                                                .kind = DescriptorKind::Sampler,
...
struct PushConstantChecksState {
    std::optional<SchemaError> empty;
...
const mdux::spec::Register pushConstantChecks{
    "Push constant ranges are checked for size, alignment and overlap", "evidence-unit", [] {
        return speclab::Test<PushConstantChecksState>("shader-push-constant-checks")
            .Given("push constant ranges that are empty, unaligned and overlapping", [](PushConstantChecksState& state) {
                ShaderPackage package = validPackage();
...
const mdux::spec::Register packageRoundTrip{
    "A package survives write() and parse() unchanged", "evidence-unit", [] {
        return speclab::Test<PackageRoundTripState>("shader-package-round-trip")
            .Given("a package with two modules, a descriptor and a push constant range",
                   [](PackageRoundTripState& state) {
                       ShaderPackage package = validPackage();
                       package.modules[0].byteLength = 32;
                       package.modules.push_back(ShaderModule{.id = "ui.frag",
                                                              .stage = Stage::Fragment,
                       ...
                       Assertions::require(text.has_value(), "write() rejected a valid package");
                       state.text = *text;

                       auto parsed = ShaderPackage::parse(*state.text);
                       Assertions::require(parsed.has_value(), "parse() rejected its own output");
                       state.parsed = *parsed;
            ...
            .Then("every field survives and the bytes are reproduced exactly",
                  [](PackageRoundTripState& state) {
                      ...
                      checks.expect(package.sidecarSha256 == digestOf("sidecar"),
                                    "sidecar digest");
                      Assertions::require(package.modules.size() == 2, "expected 2 modules, got {}", package.modules.size());
...
const mdux::spec::Register malformedTextRejected{
    "Parsing rejects malformed and semantically invalid text", "evidence-unit", [] {
        return speclab::Test<MalformedTextRejectedState>("shader-malformed-text-rejected")
            .Given("text that is not JSON, not an object, and not a package", [](MalformedTextRejectedState& state) {
                auto notJson = ShaderPackage::parse("{ this is not json");
                Assertions::require(!notJson.has_value(), "parse() accepted text that is not JSON");
                state.notJson = notJson.error();

                auto notAnObject = ShaderPackage::parse("[]");
                Assertions::require(!notAnObject.has_value(), "parse() accepted a non-object");
                ...
                Assertions::require(!wrongShape.has_value(), "parse() accepted a non-package object");
                state.wrongShape = wrongShape.error();
            })
            .When("each is parsed", [] {})
            .Then("each is rejected as MalformedPackage",
                  [](MalformedTextRejectedState& state) {
                      mdux::spec::Checks checks;
...
const mdux::spec::Register parseRunsValidate{
    "Parsing runs validate(), so an invalid package cannot be read back", "evidence-unit", [] {
        return speclab::Test<ParseRunsValidateState>("shader-parse-runs-validate")
            .Given("a package file edited into an invalid state", [](ParseRunsValidateState& state) {
                ...
                ShaderPackage package = validPackage();
                auto text = package.write();
                Assertions::require(text.has_value(), "write() rejected the reference package");

                std::string corrupted = *text;
                const std::size_t position = corrupted.find("\"byteLength\": 64");
                Assertions::require(position != std::string::npos, "the reference JSON did not contain the expected byteLength");
                corrupted.replace(position, std::string_view{"\"byteLength\": 64"}.size(),
                                  "\"byteLength\": 99");

                auto parsed = ShaderPackage::parse(corrupted);
                Assertions::require(!parsed.has_value(), "parse() accepted a semantically invalid package");
                state.error = parsed.error();
            })
            .When("it is parsed", [] {})
            .Then("it is rejected as UnalignedModule",
                  [](ParseRunsValidateState& state) {
                      mdux::spec::Checks checks;
...
const mdux::spec::Register findLocatesModule{
    "find() locates a module by id and reports a miss", "evidence-unit", [] {
        return speclab::Test<FindLocatesModuleState>("shader-find-locates-module")
            .Given("the reference package", [](FindLocatesModuleState& state) {
            ...
            .Then("the known id is found and the unknown is a miss",
                  [](FindLocatesModuleState& state) {
                      Assertions::require(state.found != nullptr, "find('ui.vert') returned nullptr");
                      mdux::spec::Checks checks;

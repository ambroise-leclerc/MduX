...
| Live-data components | Implemented | a `NumericDisplay` and a `SignalTrace` paint the field they reserve ([#255](https://github.com/ambroise-leclerc/MduX/issues/255)) and the live content inside it: a **waveform** expanded from a caller-owned ring buffer ([#257](https://github.com/ambroise-leclerc/MduX/issues/257)), and **digits** substituted into a pattern whose widest rendering the compiler measured against the node ([#258](https://github.com/ambroise-leclerc/MduX/issues/258)). A `Clock` draws too. Both refuse rather than truncate — an over-long ring, a value with more digits than its slots — and neither allocates in a frame |
...
| **Not started** | | |
| Content components (`Image`, `StatusIndicator`, `TextInput`, `Button`) | Planned | [#17](https://github.com/ambroise-leclerc/MduX/issues/17). `SignalTrace`, `NumericDisplay` and `Clock` have landed ([#257](https://github.com/ambroise-leclerc/MduX/issues/257), [#258](https://github.com/ambroise-leclerc/MduX/issues/258)) and are in the live-data row above |

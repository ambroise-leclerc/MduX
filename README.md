...
| Machine-readable `.medui` contract | Implemented | `mdux-meduic --grammar` emits the language as canonical JSON — tokens, productions, the component dictionary, field domains, the closed named-value sets, theme tokens and every diagnostic — read from the compiler's own tables rather than restated, and committed as [`docs/medui/grammar.json`](docs/medui/grammar.json). `mdux-meduic --explain MEDUI-E034` answers for one code and exits non-zero on one that is not registered ([#263](https://github.com/ambroise-leclerc/MduX/issues/263)) |
...
| Text and glyph rendering | Implemented | host-side shaping into baked runs, an R8 coverage atlas, and a governed draw path; a compiled screen's `Label` is joined to a text package at run time and drawn ([#14](https://github.com/ambroise-leclerc/MduX/issues/14), [#242](https://github.com/ambroise-leclerc/MduX/issues/242)). A `Button`'s label is drawn over its face since [#261](https://github.com/ambroise-leclerc/MduX/issues/261), which is where the rest of that sentence — a button is more than its text — was settled |
| Live-data components | Implemented | a `NumericDisplay` and a `SignalTrace` paint the field they reserve ([#255](https://github.com/ambroise-leclerc/MduX/issues/255)) and the live content inside it: a **waveform** expanded from a caller-owned ring buffer ([#257](https://github.com/ambroise-leclerc/MduX/issues/257)), and **digits** substituted into a pattern whose widest rendering the compiler measured against the node ([#258](https://github.com/ambroise-leclerc/MduX/issues/258)). A `Clock` draws too; a `StatusIndicator` draws the **state** it is bound to, out of a list the compiler closed ([#259](https://github.com/ambroise-leclerc/MduX/issues/259)); and a `TextInput` draws its **value and caret** on a fixed-pitch grid whose worst case the compiler measured against the box ([#260](https://github.com/ambroise-leclerc/MduX/issues/260)) — display only, since #17 cuts input-method editing. All refuse rather than approximate — an over-long ring, a value with more digits than its slots, a state index the list does not have, a value longer than its field — and none allocates in a frame |
| Controls (`Button`, `CriticalButton`) | Implemented | [#261](https://github.com/ambroise-leclerc/MduX/issues/261). Both draw a **face** — the whole rectangle in the one token the node carries — with the label's word over it once a locale is bound. `resolvePress()` turns a surface coordinate into the control under it, the **requirement it is traced to**, and either a member of the closed `SystemEvent` set or a `Button`'s product action name. A critical control naming an action outside that set, or declaring no requirement, refuses the press rather than reporting a no-op nothing performs. This completes the component dictionary ([#17](https://github.com/ambroise-leclerc/MduX/issues/17)) |
| Host-tool manifest | Implemented | [`docs/tools/manifest.json`](docs/tools/manifest.json) lists every host tool with its entry point, command grammar, options, linked library, diagnostic-code family and the artifacts it bakes — **generated** from `add_executable()` and the `mdux_bake_artifact()` call sites rather than written, so a tool added to the build without regenerating fails CI ([#265](https://github.com/ambroise-leclerc/MduX/issues/265)) |
| `mdux-meduic --dump-ir` | Implemented | the compiler's resolved intermediate representation as canonical JSON: the bounded box tree after layout, every authored field with its value, the RGBA each `Theme.Colors.<Token>` resolves to as `u32` bit patterns, and the text budget each node was measured against ([#265](https://github.com/ambroise-leclerc/MduX/issues/265)) |
| Recipe schemas | Implemented | one JSON Schema per baked kind under [`docs/recipes/`](docs/recipes/), documenting the **resolved** option set each baker records in `report.json` — defaults expanded, per ADR-007 decision 4. Validated against every committed report in both directions by `check_schema_type_drift.py`, so a baker that adds an option and not the schema fails CI ([#264](https://github.com/ambroise-leclerc/MduX/issues/264)) |

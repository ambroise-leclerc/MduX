...
| Live-data components | Implemented | a `NumericDisplay` and a `SignalTrace` paint the field they reserve ([#255](https://github.com/ambroise-leclerc/MduX/issues/255)) and the live content inside it: a **waveform** expanded from a caller-owned ring buffer ([#257](https://github.com/ambroise-leclerc/MduX/issues/257)), and **digits** substituted into a pattern whose widest rendering the compiler measured against the node ([#258](https://github.com/ambroise-leclerc/MduX/issues/258)). A `Clock` draws too, and a `StatusIndicator` draws the **state** it is bound to, out of a list the compiler closed ([#259](https://github.com/ambroise-leclerc/MduX/issues/259)). All refuse rather than approximate — an over-long ring, a value with more digits than its slots, a state index the list does not have — and none allocates in a frame |
...
| **Not started** | | |
| Content components (`TextInput`, `Button`, `CriticalButton`) | Planned | [#17](https://github.com/ambroise-leclerc/MduX/issues/17). `Image`, `SignalTrace`, `NumericDisplay`, `Clock` and `StatusIndicator` have landed ([#256](https://github.com/ambroise-leclerc/MduX/issues/256), [#257](https://github.com/ambroise-leclerc/MduX/issues/257), [#258](https://github.com/ambroise-leclerc/MduX/issues/258), [#259](https://github.com/ambroise-leclerc/MduX/issues/259)) and are in the live-data row above |
